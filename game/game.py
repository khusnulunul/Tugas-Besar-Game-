...
enemy_ship  = 'newenemy.png' 
ufo_ship = 'ufoucul.png' 
player_bullet = 'pelurubaru.png' 
enemy_bullet = 'newenemybullet.png' 
ufo_bullet = 'newenemybullet.png' 
nyawa_ship = 'nyawa.png' 
...
start_screen_sound = pygame.mixer.Sound('start_song.wav')

background_music = pygame.mixer.music.load('music_game.mp3') 
...
class Player(pygame.sprite.Sprite):
    ...
    def update(self): 
        if self.alive:
            ...
            self.rect.x = mouse[0] - 5
            self.rect.y = mouse[1] + 20
        else:
            self.alpha_duration = 0 
...
class Ufo(Enemy): 
    ...
    def shoot(self):
        if self.rect.x % 50 == 0:
            ...
            ufobullet.rect.y = self.rect.y + 30
...
class Game:
    ...
    def start_text(self):
        ...
        text = font.render('ROCKET THE SHOOTER', True, 'purple') 
        text_rect = text.get_rect(center=(s_width/2, s_height/2)) 
        screen.blit(text, text_rect)
        
        font2 = pygame.font.SysFont('Calibiri', 30)  
        text2 = font2.render('ENJOY TO THE GAME, GUYS :)', True, 'Navy') 
        text2_rect = text2.get_rect(center=(s_width/2, s_height/2+320))   
        screen.blit(text2, text2_rect)
        
    def start_screen(self):
        pygame.mixer.Sound.play(start_screen_sound) 
        self.lives = 5
        sprite_group.empty() 
        while True:
            screen.fill('pink') 
    ...
    def pause_text(self):
        ...
        text = font.render('PAUSED', True, 'pink') 
        text_rect = text.get_rect(center=(s_width/2, s_height/2)) 
        screen.blit(text, text_rect)
    ...
    def create_lives(self):
        self.live_img = pygame.image.load(nyawa_ship) 
        self.live_img = pygame.transform.scale(self.live_img, (22,23))
        n = 0
    ...
    def create_score(self): 
        ...
        text_rect = text.get_rect(center=(s_width-140, s_height-750))
        screen.blit(text, text_rect)
    ...
    def run_game(self): 
        pygame.mixer.Sound.stop(start_screen_sound) 
        ...
        while True:
            ...
            self.run_update()
            pygame.draw.rect(screen, 'purple', (0,0,s_width,40)) 
            self.create_lives()  
            self.create_score()
            for event in pygame.event.get():
                if event.type == QUIT:
